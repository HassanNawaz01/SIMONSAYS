...
/**
 * Holds paid 1v1 stakes until the game server signs the result.
 * Each wallet deposits exactly the selected stake. On a decisive result,
 * 0.5% is deducted from each stake and the remaining pool goes to the winner.
 * A zero-address result refunds both full stakes and charges no platform fee.
 */
contract SimonStakeEscrowV2 {
    uint256 public constant FEE_BPS = 50; // 0.5% per player stake
    ...
    event Deposited(bytes32 indexed matchId, address indexed player, address indexed opponent, uint256 stake, uint256 feeOnWin);
    ...
    function winnerPayout(uint256 stake) public pure returns (uint256) {
        return (stake - feeFor(stake)) * 2;
    }
    ...
    function deposit(bytes32 matchId, address opponent, uint256 stake) external payable nonReentrant {
        ...
        if (m.stake == 0) {
            ...
            require(block.timestamp <= m.createdAt + 30 minutes, "match expired");
        ...
        emit Deposited(matchId, msg.sender, opponent, stake, feeFor(stake));
    }
    ...
    function settle(bytes32 matchId, address winner, bytes calldata signature) external nonReentrant {
        ...
        if (winner == address(0)) {
            ...
            uint256 totalFee = feeFor(m.stake) * 2;
            payout = (m.stake * 2) - totalFee;
            credits[feeRecipient] += totalFee;
